...
> **Runnable scope.** Every notebook (Ch. 1–20) executes end-to-end on the
> synthetic tiny tier with no GPU, generated by `build_notebooks.py` /
> `build_scaling_notebooks.py`. Chapters 8–13 run the *real* stack in local mode
> (Daft, Ray Train/Tune, FiftyOne headless, LanceDB); Chapters 14–20 use clearly
> labelled mock/toy stand-ins for the pieces that genuinely need an API key, a
> GPU, or a cluster (the Claude proposer, real detection, the full BDD run). Each
> chapter shows the mechanics on the tiny tier and explains how to scale the same
> contract to `bdd-small` / `bdd-full`.
